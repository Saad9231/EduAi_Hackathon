...
import { Library, Download, Search, BookOpen, FileText } from 'lucide-react';

const mockBooks = [
  { id: 1, title: 'Physics (Class 10)', board: 'PTB', size: '15 MB', type: 'book' },
  { id: 2, title: 'Mathematics (Class 10)', board: 'FBISE', size: '22 MB', type: 'book' },
  { id: 3, title: 'Chemistry (Class 10)', board: 'PTB', size: '18 MB', type: 'book' },
  { id: 4, title: 'Urdu Grammar (Class 10)', board: 'FBISE', size: '8 MB', type: 'book' },
  { id: 5, title: 'Past Paper 2024 (Physics)', board: 'FBISE', size: '2 MB', type: 'resource' },
  { id: 6, title: 'Past Paper 2024 (Math)', board: 'PTB', size: '3 MB', type: 'resource' },
];
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-6 overflow-y-auto pr-2 pb-4 scrollbar-hide">
           {filteredBooks.length > 0 ? filteredBooks.map((book) => (
             <div key={book.id} className="glass-card p-5 flex flex-col group hover:border-amber-500/50 transition-colors">
                ...
                <button className="w-full mt-auto py-2.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-white text-sm font-bold transition-colors flex items-center justify-center gap-2 border border-slate-700 group-hover:border-amber-500/30 group-hover:bg-amber-500/10 group-hover:text-amber-400">
